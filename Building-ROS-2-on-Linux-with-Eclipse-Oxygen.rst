...
This tutorial is based on a clean ubuntu-16.04.2 install and eclipse oxygen with egit. It uses RTI Connext as middleware for Realtime performance. The `original Install page <Linux-Development-Setup>` is perhaps more up-to-date, so check it for info.
...
We now need a RTI license, which we get on their website. Refer to `Linux Development Setup page <Linux-Development-Setup>`. The RTI license file will be directly send per email after sign-up.
