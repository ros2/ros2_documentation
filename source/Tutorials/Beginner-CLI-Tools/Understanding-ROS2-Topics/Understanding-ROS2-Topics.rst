...
.. Note:: The rate reflects the receiving rate on subscription created by ``ros2 topic hz`` command, which might be affected by platform resources and QoS configuration, and may not exactly match the publisher rate.
...
.. Note:: The bandwidth reflects the receiving rate on subscription created by ``ros2 topic bw`` command, which might be affected by platform resources and QoS configuration, and may not exactly match the publisher's bandwidth.
