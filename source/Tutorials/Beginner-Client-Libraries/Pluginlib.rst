...
In ROS 2 Jazzy, there is no CLI command to list plugins. Plugins are discovered at runtime using the plugin description XML file registered in the ament index. To verify available plugins, inspect the plugin XML file and ensure it is correctly exported in ``CMakeLists.txt``.
Now run the node:
