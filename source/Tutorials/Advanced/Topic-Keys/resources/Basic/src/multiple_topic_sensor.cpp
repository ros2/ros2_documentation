...
class MultipleTopicSensorNode : public rclcpp::Node
{
public:

  explicit MultipleTopicSensorNode( const rclcpp::NodeOptions & options)
  : Node("multiple_topic_sensor", options)
  {
    ...
    pub_ = this->create_publisher<demo_keys_cpp::msg::SensorDataMsg>(
        "/robot/sensor_" + std::to_string(id_),
...
RCLCPP_COMPONENTS_REGISTER_NODE(demo_keys_cpp::MultipleTopicSensorNode)
